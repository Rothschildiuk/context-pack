...
use model::{AppConfig, OutputFormat, RenderContext, RepoInfo};
...
fn run() -> Result<(), CliError> {
    ...
    let output = render_stub_bundle(&config);
...
fn render_stub_bundle(config: &AppConfig) -> String {
    match config.format {
        OutputFormat::Markdown => {
            let walk_result = build_tree_summary(config);
            let git_result = git::collect(config);
            let selection_result = select_files(config, &git_result.changed_files);
            let repo = RepoInfo {
            ...
            let context = RenderContext {
                ...
                notes: build_notes(config, walk_result.notes, git_result.notes, selection_result.notes),
            };
